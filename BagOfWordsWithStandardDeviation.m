...
for person=1:15
data22=cell2mat(data.data2(person,10));
data33=cell2mat(data.data2(person,12));
deaviationslip(person,:)=std(diff(data22).');
deaviationgrip(person,:)=std(diff(data33).');
end
deaviationslip=deaviationslip.';
deaviationslip=deaviationslip(:);
deaviationgrip=deaviationgrip(:).';
deaviationgrip=deaviationgrip(:);

for i=1:size(deaviationslip,1)-w
    subdevslip(:,i)=deaviationslip(i:i+w);
    subdevgrip(:,i)=deaviationgrip(i:i+w);
 
end


both=[subgrip.';subslip.';subdevgrip.';subdevslip.'];
[idx,C]=kmeans(both,K);
%% data representation
...
CVSVMModel = crossval(SVMModel,'KFold',14)
